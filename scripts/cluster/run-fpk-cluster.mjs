...
if (buildImages) {
    const builtImages = run('node', ['scripts/cluster/build-local-images.mjs']);
    if ((builtImages.status ?? 1) !== 0) process.exit(builtImages.status ?? 1);
}

const rendered = run('node', ['scripts/cluster/render-fpk-cluster.mjs']);
if ((rendered.status ?? 1) !== 0) process.exit(rendered.status ?? 1);

const applied = run('kubectl', ['apply', '-R', '-f', 'tools/cluster/out']);
if ((applied.status ?? 1) !== 0) process.exit(applied.status ?? 1);
...
const logs = run('kubectl', ['logs', '-n', namespace, '-l', selector, '--tail=200', '--follow']);
