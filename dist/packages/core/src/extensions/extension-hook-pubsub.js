...
export function extensionHookTopic(extensionId, phase) {
...
export const ExtensionHookPubSub = Context.GenericTag("@eventiva/core/ExtensionHookPubSub");
const defaultMessageId = () => `${Date.now()}-${Math.random().toString(36).slice(2)}`;
/** Wraps a listener run with span, log, and duration metric so every hook execution is observable. */
function instrumentListenerRun(topic, messageId, run, payload) {
...
function makePubSub(ref) {
    return {
        listenTo: (topic, run) => Ref.update(ref, (m) => {
            const next = new Map(m);
            const list = next.get(topic) ?? [];
            next.set(topic, [...list, run]);
            return next;
        }),
        publish: (topic, payload, messageId) => Ref.get(ref).pipe(Effect.flatMap((m) => {
            ...
            })(Effect.all(list.map((run) => instrumentListenerRun(topic, id, run, payload)), { concurrency: "unbounded" }).pipe(Effect.asVoid));
        })),
        getTopics: () => Ref.get(ref).pipe(Effect.map((m) => Array.from(m.keys()))),
        getTopicsForPhase: (phase) => Ref.get(ref).pipe(Effect.map((m) => Array.from(m.keys()).filter((t) => t.endsWith(`/${phase}`))))
    };
}
export const ExtensionHookPubSubLive = Layer.effect(ExtensionHookPubSub, Effect.gen(function* () {
