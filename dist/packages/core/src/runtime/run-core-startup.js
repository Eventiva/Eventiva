...
export const runCoreStartupRaw = Effect.gen(function* () {
    ...
    const safeTables = Object.fromEntries(Object.entries(allTables).filter((entry) => entry[1] != null && typeof entry[1] === "object"));
    ...
    if (hasAnyCallbacks) {
        let mergedRelationsConfig;
        try {
            mergedRelationsConfig = defineRelations(safeTables, (helpers) => {
                const config = {};
            ...
            yield* Effect.logWarning(`Skipping relation resolution due to: ${e instanceof Error ? e.message : String(e)}. Tables: ${Object.keys(safeTables).join(", ")}.`);
    ...
    for (const [tableName, table] of Object.entries(safeTables)) {
        ...
        try {
            ...
            class DynamicEntity extends Base()(entityName, schema, { tableName }) {
            }
            EntityRegistry.register(entityName, DynamicEntity);
        }
        catch (e) {
            yield* Effect.logWarning(`Skipping entity for table "${tableName}" (not a Drizzle table? use a DB extension with SchemaFinalizer): ${e instanceof Error ? e.message : String(e)}`);
        }
    }
