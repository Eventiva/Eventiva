...
import { ExtensionHookPubSub, extensionHookTopic } from "../extensions/extension-hook-pubsub.js";
...
const StartupBannerOnLoadWorkflowLayer = StartupBannerOnLoadWorkflow.toLayer(Effect.fn((_payload, _executionId) => Effect.gen(function* () {
    yield* Activity.make({
...
const StartupBannerAfterCallWorkflowLayer = StartupBannerAfterCallWorkflow.toLayer(Effect.fn((payload, _executionId) => Effect.gen(function* () {
    yield* Activity.make({
        name: "core/startup-banner/afterCall",
...
const RegisterListenersLayer = Layer.effectDiscard(Effect.gen(function* () {
    yield* Effect.logInfo("Registering startup banner listeners");
    const pubsub = yield* ExtensionHookPubSub;
    yield* pubsub.listenTo(extensionHookTopic(HELLO_WORLD, "onLoad"), (_payload, messageId) => StartupBannerOnLoadWorkflow.execute({ messageId }).pipe(Effect.asVoid));
    yield* pubsub.listenTo(extensionHookTopic(HELLO_WORLD, "afterCall"), (payload, messageId) => StartupBannerAfterCallWorkflow.execute({ context: payload, messageId }).pipe(Effect.asVoid));
 ...
 */
export const StartupBannerLayer = Layer.mergeAll(StartupBannerOnLoadWorkflowLayer, StartupBannerAfterCallWorkflowLayer, RegisterListenersLayer);
