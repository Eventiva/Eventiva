 ...
 */
export function makeEntityEndpointDescriptor(entity, defaultEntityId, pathPrefix) {
 ...
 */
export function makeEntityEndpointsLayer(descriptors, options = {}) {
    ...
    const startServer = Effect.gen(function* () {
        ...
        for (const [name, EntityClass] of allRegisteredEntities.entries()) {
            if (!allDescriptors.some((d) => d.entity === EntityClass.entity)) {
                allDescriptors.push({
        ...
        const entityRpcGroupLive = HttpApiBuilder.group(EntityRpcApi, "EntityRpc", (handlers) => handlers.handle("invoke", ({ path: { pathPrefix }, payload }) => Effect.gen(function* () {
            const entry = map.get(pathPrefix);
            if (!entry)
                return { success: { error: `Unknown pathPrefix: ${pathPrefix}` } };
            const { entityId, method, payload: payloadData } = payload;
            if (typeof method !== "string")
                return { success: { error: "body.method is required" } };
            const client = entry.getClient(entityId ?? entry.defaultEntityId);
            const fn = client[method];
            if (typeof fn !== "function")
                return { success: { error: `Unknown method: ${method}` } };
            const rpc = entry.entity.protocol.requests.get(method);
            const decodeEffect = rpc?.payloadSchema != null
                ? Schema.decodeUnknown(rpc.payloadSchema)(payloadData ?? {})
                : Effect.succeed(payloadData ?? {});
            const result = yield* decodeEffect.pipe(Effect.flatMap((decoded) => fn(decoded)), Effect.map((success) => ({ success })), Effect.catchAll((err) => Effect.succeed({
                success: { error: err instanceof Error ? err.message : String(err) }
            })));
            return result;
        })));
        const shutdownResponse = { ok: true, message: "Shutting down" };
        const scheduleExit = Effect.sync(() => setTimeout(() => process.exit(0), 100));
        const shutdownGroupLive = HttpApiBuilder.group(EntityRpcApi, "Shutdown", (handlers) => handlers
        ...
        const apiLayer = HttpApiBuilder.api(EntityRpcApi).pipe(Layer.provide(entityRpcGroupLive), Layer.provide(shutdownGroupLive));
        ...
        const fullServerLayer = Layer.mergeAll(serveLayer, swaggerLayer).pipe(Layer.provide(apiLayer), Layer.provide(NodeHttpServer.layerContext));
