 ...
 */
export const encryptedString = Schema.transformOrFail(Schema.String, Schema.String, {
    decode: (fromA, _options, ast, _fromI) => PiiEncryption.pipe(Effect.flatMap((pii) => pii.decrypt(fromA)), withSpanAndLog("schemaEncryption.decode"), Effect.mapError((e) => new ParseResult.Type(ast, fromA, encryptionMessage(e)))),
    encode: (toI, _options, ast, toA) => PiiEncryption.pipe(Effect.flatMap((pii) => pii.encrypt(toA)), withSpanAndLog("schemaEncryption.encode"), Effect.mapError((e) => new ParseResult.Type(ast, toI, encryptionMessage(e))))
});
