...
export function sayHelloHandler(envelope: Request<any>): Effect.Effect<string, never, HelloWorldConfig> {
    ...
    }).pipe(withSpanAndLog('sayHelloHandler', { attributes: { entityId } })) as Effect.Effect<
        string,
        never,
        HelloWorldConfig
    >;
