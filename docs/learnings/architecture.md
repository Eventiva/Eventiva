...
- **Platform template.** Runnable platforms live under `packages/platforms/*` (e.g. **`packages/platforms/postgresql`**, **`packages/platforms/sqlite-demo`**). Each entry composes extensions, database stack, and HTTP via **`createPlatformTemplateTwoPhase`** (see `packages/core/src/runtime/platform.ts`). Runner profiles (e.g. `defaultProfile = [HelloWorldLayer]`) name a set of entity layers for one runner process.
...
- **Manifests.** FPK/Kubernetes cluster manifests are generated from `packages/cluster-tooling/fpk/src` components: `pg`, `shard-manager`, `runner`, `workload`.
...
- **Regression gate.** `packages/cluster-tooling/cli/pg-e2e-via-nx.ts` is the PostgreSQL regression gate and runs against `platforms-postgresql:run` (cluster mode path), not a separate local-only transport path.
...
**Usage.** Use **createPlatformTemplateTwoPhase(options)** to get `getBootstrapLayer()` and `getRuntimeLayer()`, and **runMainTwoPhase(template)** to run bootstrap then runtime in one process. The PostgreSQL platform (`packages/platforms/postgresql`) uses this so `/api/rpc/contacts` and `/api/docs` expose Contact. Legacy **createPlatformTemplate** + **runMain** remain for backward compatibility but may miss dynamic entities in the route map.

### Platform packages: no shared `packages/platforms/*` library

- **Do not** add a workspace package whose sole job is to share `extensions.ts`, `platform.ts` wiring, or other “platform glue” across `packages/platforms/*`. Platform folders are **deliberately duplicated**: each runnable platform should **mirror the same file set** as the others (e.g. thin `index.ts` that only calls `runPlatform`, **`platform.ts`** that builds `createPlatformTemplateTwoPhase(...)`, `extensions.ts`, `register-database-backends.ts`, tsconfig/run wiring). The **only** intentional differences are **`register-database-backends.ts`** and the **`platform.ts`** activation line (`activateDatabaseStackFromEnv` vs `activateDatabaseBackend('…')`).
- **Why.** Platforms are composition roots and product choices; hiding differences behind a shared package makes drift invisible and encourages hidden coupling. Duplication keeps each platform self-contained and easy to diff.
- **Contrast.** Sharing **database** facades in `@eventiva/databases.shared` is correct; sharing **platform** sources is not the same pattern.
