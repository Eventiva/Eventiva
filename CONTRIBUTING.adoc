...
**Critical rule for AI-assisted development:**
* **Never** clone, initialize, or access the tests repository when doing implementation work with AI tools.
* The tests repository should only be accessed when:
  * Running tests locally (use the test-runner devcontainer)
  * Writing or reviewing tests
  * Working in a dedicated test-writing environment

For normal development work (implementation, features, bug fixes), keep the `tests/` folder unpopulated.
This ensures AI tools don't use test code as context when implementing features, which would lead to code that "beats the tests" instead of satisfying the real contract.

See `docs/learnings/tdd-and-test-creation.md` for the full TDD policy and `docs/learnings/effect-vitest-testing.md` for how to write Effect/Vitest tests.
...
- Nx - Utilized to build, test, lint, and type-check all affected packages in the monorepo.
...
- Build and Test: Nx will automatically build all affected packages, run linting, type checking, and tests.
This ensures that your changes don't break existing functionality and that all packages in the monorepo remain compatible.
- Checks: A number of security, code and build steps will run on your branch.
