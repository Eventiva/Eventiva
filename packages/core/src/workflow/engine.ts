...
class WorkflowRegistryImpl implements WorkflowRegistryService {
  ...
  constructor(private readonly registry: Ref.Ref<Map<string, (opts: WorkflowExecuteOptions<unknown>) => Effect.Effect<unknown, unknown>>>) {}
  ...
  register<Payload, Result, Error, R>(
    ...
    return withSpanAndLog(`workflow.register`, { attributes: { workflow: workflow.name } })(
      Effect.flatMap(Ref.get(this.registry), (map) =>
        Ref.set(
          this.registry,
          new Map(map).set(workflow.name, executeFn as (opts: WorkflowExecuteOptions<unknown>) => Effect.Effect<unknown, unknown>)
        )
      )
