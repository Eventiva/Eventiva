...
export function makeExtensionWorkflowLayer<R = never, E = never>(
  ...
  const workflowLayer = workflow.toLayer(
    Effect.fn((_payload: Payload, _executionId: string) => Effect.gen(function* () {
      ...
      yield* hooks.publish(extensionHookTopic(extensionId, "beforeCall"), ctx).pipe(
      ...
      yield* hooks.publish(extensionHookTopic(extensionId, phase as HookPhase), _payload.context).pipe(
        Effect.catchAll(() => Effect.void)
      )

      yield* hooks.publish(extensionHookTopic(extensionId, "afterCall"), ctx).pipe(
        Effect.catchAll(() => Effect.void)
      )
    }).pipe(withSpanAndLog("extensionWorkflowLayer", { attributes: { extensionId, phase, topic } })))
  )

  const listenerLayer = makeTopicListenerLayer(
    topic,
    (payload, messageId) => workflow.execute({ messageId, context: payload }).pipe(Effect.asVoid)
  )
...
export function withExtensionHooksWith<A, E, R, Req extends { address: { entityId: string }; payload: unknown }>(
  ...
  return (request) => {
    ...
    return eff.pipe(withSpanAndLog("withExtensionHooksWith", { attributes: { extensionId, entityType, method } })) as Effect.Effect<A, E, R>
  }
}
