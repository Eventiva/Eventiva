...
export function withExtensionHooksWith<A, E, R, Req extends { address: { entityId: string }; payload: unknown }>(
  ...
  return (request) =>
    Effect.gen(function* () {
      ...
      yield* hooks.publish(extensionHookTopic(extensionId, "beforeCall"), ctx) as Effect.Effect<void, E, unknown>
      const result = yield* handler(request)
      yield* hooks.publish(extensionHookTopic(extensionId, "afterCall"), ctx) as Effect.Effect<void, E, unknown>
      return result
 ...
 */
export function withExtensionHooks<A, E, R, Req extends { address: { entityId: string }; payload: unknown }>(
  extensionId: string,
  entityType: string,
  method: string,
  handler: (request: Req) => Effect.Effect<A, E, R>
): (request: Req) => Effect.Effect<A, E, R | ExtensionHookPubSub> {
  return (request) =>
    Effect.gen(function* () {
      ...
      yield* hooks.publish(extensionHookTopic(extensionId, "afterCall"), ctx) as Effect.Effect<void, E, unknown>
      return result
    }) as Effect.Effect<A, E, R | ExtensionHookPubSub>
}
 ...
 */
export const runCoreStartup = Effect.gen(function* () {
  ...
  void,
  { _tag: "IntegrityCheckFailed"; reason: string },
  ExtensionHookPubSub
>
