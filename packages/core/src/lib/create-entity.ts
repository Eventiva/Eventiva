...
import { makeCrudEntity } from "./crud-rpc.js"
import { makeCrudHandlersFromDatabase } from "./crud-handlers.js"
import { Database } from "./database.js"
import { runWithExtensions } from "./entity-method-extensions.js"
import { typeId, typeIdSchema } from "./typeid-schema.js"
 ...
 */
export function createEntity<Fields extends Record<string, unknown>>(
  options: CreateEntityOptions<Fields>
): CreateEntityResult<
  string,
  Schema.Schema.Type<ReturnType<typeof typeIdSchema>>,
  Fields,
  import("@effect/rpc/Rpc").Any
> {
  const { name, schema, withDelete = false } = options
  const tableName = options.tableName ?? name
  const idPrefix = name.toLowerCase() as string
  const idSchema = typeIdSchema(idPrefix)
  type Id = Schema.Schema.Type<typeof idSchema>
  const recordSchema = Schema.Struct({ id: idSchema }).pipe(Schema.extend(schema)) as Schema.Schema<
    { readonly id: Id } & Fields,
    { readonly id: Id } & Schema.Schema.Encoded<typeof schema>,
    Schema.Schema.Context<typeof schema>
  >
  const fieldsSchema = schema as Schema.Schema<Fields, unknown, never>
  ...
  const handlers = makeCrudHandlersFromDatabase({
    ...
    fieldsSchema,
    recordSchema,
    genId: () => typeId(idPrefix) as Id,
    withDelete,
    _rpcUnion: entity.protocol as unknown as import("@effect/rpc/Rpc").Any
  })

  const wrap = <A, E, R>(method: string, h: (req: any) => Effect.Effect<A, E, R>) => (req: any) =>
    runWithExtensions(name, method, h, req)
  ...
  const wrappedHandlers = {
    ...
    ...(handlers.delete ? { delete: wrap("delete", handlers.delete) } : {})
  }

  const layer = entity.toLayer(Effect.succeed(wrappedHandlers as any))

  return {
    entity: entity as Entity<string, import("@effect/rpc/Rpc").Any>,
    layer
  } as CreateEntityResult<string, Id, Fields, import("@effect/rpc/Rpc").Any>
}
