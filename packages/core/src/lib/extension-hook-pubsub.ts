...
function instrumentListenerRun(
  ...
  return withSpanAndLog("extension_hooks.listener", {
    ...
    run(payload, messageId).pipe(Effect.catchAll(() => Effect.void))
  ).pipe(Effect.asVoid)
}

function makePubSub(ref: Ref.Ref<Map<string, Array<HookListenerRun>>>): ExtensionHookPubSub {
  const publishToPhase = (phase: HookPhase, payload: unknown) =>
    Ref.get(ref).pipe(
      Effect.flatMap((m) => {
        const topics = Array.from(m.keys()).filter((t) => t.endsWith(`/${phase}`))
        if (topics.length === 0) return Effect.void
        const messageId = defaultMessageId()
        return Effect.all(
          topics.map((topic) => {
            const list = m.get(topic) ?? []
            return Effect.all(
              list.map((run) => instrumentListenerRun(topic, messageId, run, payload)),
  ...
  return {
    ...
    getTopics: () =>
      Ref.get(ref).pipe(Effect.map((m) => Array.from(m.keys()))),

    getTopicsForPhase: (phase) =>
      Ref.get(ref).pipe(
        Effect.map((m) =>
          Array.from(m.keys()).filter((t) => t.endsWith(`/${phase}`))
        )
      ),

    publishOnLoad: () => publishToPhase("onLoad", {}),
    publishOnRegister: (ctx) => publishToPhase("onRegister", ctx),
    publishBeforeCall: (ctx) => publishToPhase("beforeCall", ctx),
    publishAfterCall: (ctx) => publishToPhase("afterCall", ctx),
    publishShutdown: () =>
      Ref.get(ref).pipe(
        Effect.flatMap((m) => {
          const list = m.get(CORE_SHUTDOWN_TOPIC) ?? []
          if (list.length === 0) return Effect.void
          const id = defaultMessageId()
          return withSpanAndLog("extension_hooks.publish", {
            attributes: { topic: CORE_SHUTDOWN_TOPIC, messageId: id },
            metricName: "extension_hooks.publish.duration"
          })(
            Effect.all(
              list.map((run) => instrumentListenerRun(CORE_SHUTDOWN_TOPIC, id, run, {})),
              { concurrency: "unbounded" }
            ).pipe(Effect.asVoid)
          )
        })
