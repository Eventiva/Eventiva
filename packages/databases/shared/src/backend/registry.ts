...
import type { Database, RuntimeSchemaDDLService, SchemaFinalizer } from '@eventiva/core';
 ...
 *
 * - Set `dialect` when extensions use `@eventiva/databases.shared` facades for that engine.
 *   {@link activateDatabaseBackend} will call {@link installDatabaseDialect} for you.
 * - Omit `dialect` only for special backends that do not drive `@eventiva/databases.shared` column facades.
 */
