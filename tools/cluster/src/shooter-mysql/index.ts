...
/** Client-only cluster socket (`makeClusterSqlClientLayer`); no listener on 34431 — avoid TCP probes (see postgres `shooter`). */
const container = pipe(
    K.containerWithPorts(name, image, { tcp: 34431 }),
    K.setImagePullPolicy('IfNotPresent'),
    K.concatEnv({
        NODE_ENV: 'development',
        CLUSTER_APP_MODE: 'shooter',
