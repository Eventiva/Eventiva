...
>(
    ...
    create: (req: Request<ExtractTag<RpcUnion, 'create'>>) => Effect.Effect<{ id: Id }, never, Store | unknown>;
    ...
    const deleteHandler = (req: Request<ExtractTag<RpcUnion, 'delete'>>): Effect.Effect<void, NotFound<Id>, Store> =>
...
export interface CrudHandlersOptionsWithDatabase<
    ...
    Encoded extends Record<string, unknown> & { readonly id: unknown },
    RpcUnion extends import('@effect/rpc/Rpc').Any,
> {
    readonly entityType: string;
...
>(
    ...
    const decodeRecord = (stored: Encoded) => Schema.decodeUnknown(recordSchema)(stored).pipe(Effect.orDie);
    const encodeRecord = (record: EntityRecord) => Schema.encode(recordSchema)(record).pipe(Effect.orDie);
