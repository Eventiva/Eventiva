/**
 * SQLite demo platform template (no process lifecycle). Imported by `index.ts` and tests.
 */
...
import { activateDatabaseBackend } from '@eventiva/databases.shared';

const { databaseLayer, schemaFinalizerLayer, runtimeSchemaDDLLayer } = activateDatabaseBackend('sqlite');
