...
- **Type resolution in tests repo (IDE / dev):** In the tests repo, TypeScript and the IDE resolve `**@eventiva/core`**, `**@eventiva/extensions/hello-world`**, etc. to the **committed dist** (e.g. `dist/packages/core`) so that imports type-check and navigate without implementation code – types only, per [tdd-and-test-creation.md](docs/learnings/tdd-and-test-creation.md).
- **Resolution when running tests (main repo):** When the main repo runs tests (Job 3), it clones the tests repo into main’s `**tests/`** folder. Nx in the main repo discovers projects under `**tests/`** (e.g. `tests/core`, `tests/extensions/hello-world`) and runs their test target. Module resolution for `**@eventiva/*`** at test-run time must point at the **main repo’s implementation** (main’s `dist/` or built packages), not at `tests/dist/`, so tests execute against real code.
...
- **Nx behaves as if tests were in the main repo:** Nx is configured so that projects under `**tests/`** are first-class workspace projects (e.g. include `**tests/**/project.json`** or equivalent so that `tests/core`, `tests/extensions/hello-world` are discovered). Then you can run `**nx run tests-core:test`**, `**nx run tests-extensions-hello-world:test`**, or `**nx run-many -t test**` and have Nx run both in-repo tests (if any) and the tests from the tests repo. No special “tdd” target needed – the test target is the same as for other packages; test projects are just named `tests-<path>` and live under `tests/`.
