...
The main backend entry point is the `platforms-postgresql` Nx project. From the repository root, after dependencies are installed (and ideally after a successful build), start it with:

[source,bash]
----
npx nx run platforms-postgresql:run
----

This target builds the platform library (if needed) and executes `packages/platforms/postgresql/src/index.ts` via `tsx`.
The process stays running (HTTP entity endpoints on port 3000 by default, or `EVENTIVA_HTTP_PORT` if set); stop it with kbd:[Ctrl+C].
If Nx reports a non-zero exit after you interrupt or kill the process, that is expected.

If you use pnpm as the package manager (recommended for this repo), the equivalent is:
...
* `npx nx run platforms-postgresql:run` - **Run the PostgreSQL platform** (backend entry point; builds first via target `dependsOn`)
