...
import { DatabaseAspect, DatabaseNode } from '@eventiva/utilities.database'
import { buildSchema } from 'drizzle-graphql'
import { DocumentNode, parse, printSchema } from 'graphql'
import type { GraphqlConfig } from './graphql-config.js'
...
export class GraphqlNode {
    ...
    static async provider (
        ...
        const graphql = new GraphqlNode( config )

        const { schema } = buildSchema( databaseNode.db! )

        // Convert the schema to SDL
        const typeDefsSDL: string = printSchema( schema )
        // Parse the SDL to DocumentNode
        const typeDefs: DocumentNode = parse( typeDefsSDL )
