...
import {AppLayout} from '@bitdesign/sparks.layout.app-layout';
import {Route, Routes} from 'react-router-dom';
import type {Route as RouteType} from '@bitdev/symphony.frontends.route';
import {LayoutMap} from './layout-map.js';
...
export type PlatformAppProps = {
     ...
     * top level
     */
    TopLevel: ComponentType<{ children: ReactNode }>;

    /**
     * a component for showing not found.
     */
...
export function PlatformApp({routes, layout, TopLevel, NotFoundComponent, hud}: PlatformAppProps) {
    const {top, bottom, right, left} = layout;
    const defaultNode = () => <></>;
    ...
    return (
            <TopLevel>
                {hud}
                <AppLayout
                        right={right ? right.component : defaultNode}
                        left={left ? left.component : defaultNode}
                        top={top ? top.component : defaultNode}
                        bottom={bottom ? bottom.component : defaultNode}
                >
                    <Routes>
                        {routes.map((route) => {
