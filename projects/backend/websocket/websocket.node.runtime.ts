/*
 * Project: Eventiva
 * File: websocket.node.runtime.ts
...
import { BackendContext, BackendServer } from '@bitdev/symphony.backends.backend-server'
...
import type { WebsocketConfig } from './websocket-config.js'
...
export class WebsocketNode
    ...
    static defaultConfig: WebsocketConfig = {}
    ...
    static async provider (
        [ symphonyPlatform, databaseNode ]: [ SymphonyPlatformNode, DatabaseNode ],
        config: WebsocketConfig
    ) {
        const websocket = new WebsocketNode( config )


        return websocket
    }

    async run ( { name, ...context }: BackendContext ): Promise<ApplicationInstance> {
