 ...
 * Path: \projects\workflows\generators\discord\bot\discord-bot-route\app-template.ts
...
export type DiscordBotRouteTemplateOptions = Partial<Pick<ComponentTemplate, 'name' | 'description' | 'hidden' | 'env'>>;

export class DiscordBotRouteTemplate implements ComponentTemplate {
  constructor(
    readonly name = 'discord-bot-route',
    readonly description = 'a template for generating an discord bot api route',
    readonly hidden = false,
    readonly env?: string
  ) {}

  generateFiles(context: ComponentContext): ComponentFile[] {
    return [indexFile(context), mainFile(context), testFile(context)];
  }

  static from(options: DiscordBotRouteTemplateOptions = {}) {
    return () =>
      new DiscordBotRouteTemplate(options.name, options.description, options.hidden, options.env);
  }
}
