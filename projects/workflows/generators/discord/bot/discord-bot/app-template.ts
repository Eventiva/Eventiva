...
export type DiscordBotTemplateOptions = Partial<Pick<ComponentTemplate, 'name' | 'description' | 'hidden' | 'env'>>;
...
export class DiscordBotTemplate implements ComponentTemplate {
  ...
  static from(options: DiscordBotTemplateOptions = {}) {
    return () =>
      new DiscordBotTemplate(options.name, options.description, options.hidden, options.env);
  }
}
