...
import { SymphonyAspect } from './symphony.aspect.js'
import { SymphonyNode } from './symphony.node.runtime'

it.skip( 'should retrieve the aspect', async () => {
    const symphony = await loadAspect<SymphonyNode>( SymphonyAspect, {
        runtime: 'node'
    } )

    expect( symphony ).toBeTruthy()
} )
