 ...
 */
import type { EmptyObject } from '@eventiva/utilities.helpers.common'
import type { HandlingRules, SchemaHandler } from '@eventiva/utilities.helpers.schema-walker'
import { z } from 'zod'
import { onElective } from './on-elective.js'
import { onIntersection } from './on-intersection.js'
import { onSomeUnion } from './on-some-union.js'

export type Check = SchemaHandler<boolean>;

export const checks: HandlingRules<boolean, EmptyObject, z.ZodFirstPartyTypeKind> = {
    ZodObject: (
    ...
    ZodRecord: (
        { valueSchema }: z.ZodRecord,
        { next }
    ) => next( valueSchema ),
    ZodArray: (
        { element }: z.ZodArray<z.ZodTypeAny>,
        { next }
    ) => next( element ),
    ZodDefault: (
