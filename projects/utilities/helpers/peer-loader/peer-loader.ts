...
export async function peerLoader<T> (
    moduleName: string,
...
export const moduleLoader = async ( modulesRaw: string[] ): Promise<Record<string, unknown>> => {
    const modules = await Promise.all( modulesRaw.map( async module => {
